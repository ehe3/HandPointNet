...
class FootPointDataset(data.Dataset):
    def __init__(self, root_path, opt, train=True):
        ...
        if self.train:  # train
            for cur_data_dir, _, _ in sorted(os.walk(data_root)):
                if cur_data_dir == data_root:
                    continue
                cur_index = int(os.path.basename(cur_data_dir))
                if cur_index == test_index:
                    continue
                print("Training: " + cur_data_dir)
                self.__loaddata(cur_data_dir)
        else:  # test
            cur_data_dir = os.path.join(self.root_path, str(test_index).zfill(4))
